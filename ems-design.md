...
> **CEL-translation note (control plane):** legacy filter matching is **fully case-insensitive** (whole event JSON, path, and value lower-cased) and a trailing `.*` is a `Regex.quote(prefix)+".*"` **literal-prefix** match, not free regex ([old-ems/JsonFilterRuleset.scala:14-31](old-ems/JsonFilterRuleset.scala)). §7's "the legacy value is a regex" overstates it. Subscription CEL + the Normalizer must reproduce case-insensitive comparison; the `TOPSIDE.*` clause is a prefix match. **Refined by A13 below** — the Normalizer as built does *not* reproduce it — and then **satisfied by A15**, which folds the CEL activation view to lower-case (keys and string values) exactly as legacy did, so the legacy literals transfer verbatim.

**Ops-readiness amendments (agreed 2026-07-28, Phase 4 grounding).** Recorded before any Phase-4 code was written, per the amendment protocol. A11 comes from reading the build's own config against its own classpath; A12–A14 come from re-reading `old-ems/` for the `seed-0` translation; A15 is a product decision taken at the Batch-0 checkpoint (rule text is owned by DAG authors, so it must stay simple) that supersedes A13's remedy. These win over earlier revisions of *this* document and over the trigger plan.

| # | Amendment | Replaces | Grounding & rationale |
|---|-----------|----------|-----------------------|
| A11 | **The observability transport is Prometheus *pull*.** Metrics are scraped from `/actuator/prometheus` via a `ServiceMonitor`, and the §10 alert column ships as a `PrometheusRule` **inside the Helm chart**. `micrometer-registry-prometheus` joins the classpath; `micrometer-registry-otlp` stays but exports **off by default** (`management.otlp.metrics.export.enabled: false`), one property away for an environment with a collector | §10 "Observability (**Micrometer → OpenTelemetry**)"; the implied OTLP-push-only transport | The config and the classpath contradict each other today: [`ems/src/main/resources/application.yml:47`](ems/src/main/resources/application.yml) exposes the `prometheus` actuator endpoint, but [`ems/pom.xml:126-129`](ems/pom.xml) puts only `micrometer-registry-otlp` on the classpath — Boot registers `/actuator/prometheus` only when the Prometheus registry is present, so **the exposed endpoint does not exist**. One of the two had to give. Pull wins because the §10 alert column is only deliverable *as code* against a Prometheus that scrapes; a `ServiceMonitor` shipped beside an OTLP-push-only app would be a chart that silently never collects. **Naming consequence, measured (`MetricNamingTest`):** the Prometheus client reserves `_info` as the Info-metric suffix and **strips** it from a gauge, so the table row above reads `ems_registry_version{component,version}` — the `_info` spelling cannot be scraped. Counter names already ending in `_total`, and a `baseUnit("seconds")` gauge already ending in `_seconds`, are **not** double-suffixed; the other nine §10 names are exposition-exact |
| A12 | **The legacy `filter.post` property and the `post_filter_control_dag_map` table are NOT redundant — they are two inputs with two different jobs.** The **property** is a second *admission* (persist) input; the **map** is the *routing* authority. Consequently `PERSIST ⊇ FORWARD` (§7) is a **hard correctness requirement** of the `seed-0` translation, not a CI nicety | §7 "Redundancy to resolve during the seed audit: … confirm the map is what `EventFilter.scala` actually evaluates, **then retire the property**"; §14 item 3's framing of the same question (**now partially answered**) | The Kafka admission gate is `!(filterPersist ‖ filterPost)` ([old-ems/EventFilter.SCALA:68-70](old-ems/EventFilter.SCALA)), and `filterPost` evaluates `postFilterRuleset` = the **property** ([:56-60](old-ems/EventFilter.SCALA), bound at [:24](old-ems/EventFilter.SCALA)). Routing dag ids come from a *different* method over a *different* ruleset: `filterPostWithDagIds` → `postFilterRulesWithDagId` = the **map** ([:35-50](old-ems/EventFilter.SCALA), bound at [:26-27](old-ems/EventFilter.SCALA)). Retiring the property would therefore have narrowed what legacy *persisted*, not just tidied config. Further: [old-ems/properties.sql](old-ems/properties.sql) defines **no** `eventorchestration.filter.post` row at all, so on the only inventory evidence available the effective legacy admission set is the **7 persist rows alone** — and a FORWARD (map) row that matches an event no PERSIST row admits is simply **dead**. The §14-item-3 residual is now only the per-environment row deltas |
| A13 | **CEL is case-sensitive in keys *and* values; the legacy engine was case-insensitive in both. The `seed-0` rules must therefore make the fold explicit** — ~~`.lowerAscii()` on both sides for every field the `Normalizer` does not canonicalize, and a `has()`-guarded either-spelling expression wherever the legacy sources disagree on key case~~. The `Normalizer` is **not** extended to compensate. **Remedy superseded by A15** (the fold moves out of the rule text and into the activation view); A13's *finding* — CEL is case-sensitive and the `Normalizer` does not compensate — stands, and is the reason A15 exists | [`ems/src/test/resources/fixtures/subscriptions_seed0.provenance.md:20-22,31-34`](ems/src/test/resources/fixtures/subscriptions_seed0.provenance.md) ("the Normalizer canonicalizes enumerated fields to upper-case at eval time … These differ only in case; the legacy engine and the future Normalizer treat them identically") | Legacy lower-cases the **whole event JSON** ([old-ems/JsonFilterRuleset.scala:17](old-ems/JsonFilterRuleset.scala)) **and the JSONPath** ([:22](old-ems/JsonFilterRuleset.scala)), so neither value case nor key case mattered. [`Normalizer.normalizeEvent`](ems/src/main/java/com/orchestration/ems/ingestion/Normalizer.java) (`Normalizer.java:152-177`) upper-cases **only** `additionalData.STATE`, `additionalData.TYPE\|type` and `taskEventType` — it never touches `tenant`, `msgTypeEventType`, `updateType`, `batchtype/batchType`, `RUN_TYPE`, `FREQUENCY` or `source`, and it never touches **keys** at all. Two casualties in the fixture as written: `event.additionalData.msgTypeEventType == "DATA-UPDATE"` can never match the legacy literal `data-update`, and PERSIST's `batchtype` vs FORWARD's `batchType` cannot both resolve. Widening the `Normalizer` instead was rejected: `ems_normalization_mutations_total` must stay reviewed-and-≈-zero before cutover (§4.4), and folding ordinary routing fields into it would destroy that signal |
| A14 | **The FORWARD half of the `seed-0` translation is an interpretation requiring human sign-off, not a mechanical port.** Every departure from the literal legacy text is enumerated and signed off in [`docs/ems-seed0-assumptions.md`](docs/ems-seed0-assumptions.md). (The PERSIST half **remains mechanical** — it derives from a well-formed JSON array) | §7 "**Mechanical translation** (legacy flat JSONPath-equality…)" as applied to FORWARD rows; §11 stage 0 item 3 "**mechanically translated** to CEL per §7" | The map-table condition grammar has no readable parser in the workspace: [old-ems/EventFilter.SCALA:86](old-ems/EventFilter.SCALA) calls `PostFilterControlDagMapItem.parseConditionString`, and **that source is not present in `old-ems/`**. The rows themselves are malformed pseudo-SQL with interleaved column-name/value tuples, and row 1's condition mixes `.` and `,` as separators (`'$.additionalData.tenant:FRCA.msgTypeEventType:data-update,'` — [old-ems/properties.sql:41-42](old-ems/properties.sql)), on top of the `MERYVAL` / `context.date.*` typos §7 already flags. A translation cannot be called mechanical when the grammar it must invert is unreadable |
| A15 | **Subscription CEL evaluates against a case-folded *matching view*: every object key and every string value in the activation tree is lower-cased, recursively.** Rules therefore have exactly one spelling — all-lowercase paths, all-lowercase literals, plain `==`, no `.lowerAscii()`, no `has()` guards, no either-spelling branches. The fold is a **routing-path view only**: the stored JSONB, the promoted/generated columns, `/run/status`, `/gate/groups` and the outgoing Airflow `conf` are all untouched by it, and it is **not** part of `Normalizer` — so `ems_normalization_mutations_total` keeps its reviewed-and-≈-zero meaning (§4.4) | A13's remedy ("the `seed-0` rules must make the fold explicit … `.lowerAscii()` on both sides … `has()`-guarded either-spelling expression") | **Decision (2026-07-28): subscription rules are authored and maintained by DAG authors, so the simplest possible rule surface outranks fidelity to the payload's key casing.** The fold is also the *more* faithful port: legacy lower-cases the whole event JSON ([old-ems/JsonFilterRuleset.scala:17](old-ems/JsonFilterRuleset.scala)) **and** the JSONPath ([:22](old-ems/JsonFilterRuleset.scala)), so a lower-cased activation view reproduces legacy matching exactly and lets the legacy value literals (`data-update`, `frca`, `topside*`) transfer **verbatim** — which restores the mechanical PERSIST translation and shrinks A14's interpretive surface to the FORWARD map rows alone. Two consequences to hold: (a) duplicate keys after folding (A8's `TYPE` vs `type`) collapse — resolved **last-wins in document order**, which is what legacy's textual `toLowerCase` + re-parse produced, and the fold logs a WARN on collision; (b) `Normalizer`'s upper-casing (`FINISH`, `DAILY`, `AMER`) is folded back down for matching only, so a rule compares `== "finish"` while the DB column still reads `FINISH` — one vocabulary per surface, documented in the user guide |
| A16 | **The §12 `EXPLAIN` acceptance criterion names two indexes the canonical §4.3 query cannot reach.** Under A10 that query filters raw JSONB in four locations with **no** param→column alias map, so `event.task_id` and the context triple never appear in a predicate; only an id-qualified call (`event_id`/`context_id`) reaches an index at all. `idx_event_task_id` is genuinely used — by `RunStatusRepository` (`task_id = ?`), not by `/event`. `idx_context_rep_freq_region` is currently **used by no read path**: `ContextQueryRepository` filters params in Java, verbatim from `checkRequestedParams`. **Resolution:** the §12 perf harness asserts an index only where one is reachable (`idx_event_task_id` on `/run/status`; PK / `idx_event_context_id` on the id-qualified `/event` shape) and *measures and records* the unqualified `/event` shape without asserting a plan node on it. The index set is **kept**: it is cheap, and the §4.3 canonicalization that would make the alias map reachable is deferred, evidence-gated on §14 item 1b. A10 is **not** re-opened — rewriting the read path onto generated columns would change matching semantics and break byte-compat, which is the whole point of A10 | §12 Performance: "assert the canonical query (§4.3) p95 < 50 ms **and plan uses `idx_event_task_id` / `idx_context_rep_freq_region`** (no seq scans)"; and the §12 acceptance line "seq-scan + hash-join → index nested-loop" | The read path's only predicates are `e.json->>?`, `e.json->'additionalData'->>?`, `c.json->>?`, `c.json->'data'->>?` plus the three id templates ([`ems/src/main/java/com/orchestration/ems/store/EventQueryRepository.java:46-53`](ems/src/main/java/com/orchestration/ems/store/EventQueryRepository.java)); `task_id = ?` appears only in [`RunStatusRepository.java:63`](ems/src/main/java/com/orchestration/ems/store/RunStatusRepository.java); no `WHERE` in `src/main/java` references `reporting_date`, `frequency` or `h3_region` ([`V2__indexes.sql:8,15`](ems/src/main/resources/db/migration/V2__indexes.sql)) |
| A17 | **The `post_filter_control_dag_map` condition grammar is `$.<prefix>` followed by one or more `.key:value` pairs, comma-separating independent `$.`-rooted groups, all ANDed.** Derived — not guessed — by comparing two rows that use the identical construction. Consequence: `cap_data_update.FRCA_CURATION`'s repeated `tenant:FRCA` is one group restating the prefix's first key, so collapsing it is information-preserving, and the row's team is now explicit. **A14's finding is narrowed**: the FORWARD half is a derivation from two consistent samples rather than an inference from a rule's name — still short of reading the parser, so it stays a sign-off item, but ASSUMPTION-4 (team inference) is void | A14 "the map-table condition grammar has no readable parser in the workspace", as the basis for treating FRCA_CURATION as an interpretation of unknown shape | `cap_data_update.FRCA_CALC` ([old-ems/properties.sql:43](old-ems/properties.sql)) reads `'$.additionalData.tenant:FRCA.updateType:CALC_EVENT,$.additionalData.STATE:FINISH'` — the same `.key:value` chaining, in a row whose intent is unambiguous. The `FRCA_CURATION` row ([:41](old-ems/properties.sql)) was corrected at source on 2026-08-02: its column layout now matches the other eight (`'CAPITAL', 'cap_data_update.FRCA_CURATION', <condition>, <dag>, TRUE`); the condition text is unchanged. `PostFilterControlDagMapItem.parseCondition` ([old-ems/EventFilter.SCALA:86](old-ems/EventFilter.SCALA)) remains absent |
| A18 | **The A15 matching view also aliases hyphenated keys: every object key containing `-` is additionally exposed with its hyphens removed, and a camelCase spelling wins over a hyphenated one.** A rule writes `context.data.runcategory` once instead of choosing between `run-category` and `runCategory`. Precedence is structural, not document order: a camelCase key folds *directly* onto the hyphen-free name in the first pass and the alias pass never overwrites an existing key. The hyphenated key is retained, so nothing that already resolved stops resolving. Same scope limits as A15 — routing activation only, never the stored JSONB, the generated columns, `/run/status`, `/gate/groups` or the outgoing `conf` | A15's fold as originally specified (lower-case keys and string values only), which left `run-category` and `runCategory` as two distinct keys and forced every rule over such a field to pick one spelling and silently miss the other | A8 established both spellings are live ([`V1__event_context.sql:100-108`](ems/src/main/resources/db/migration/V1__event_context.sql) COALESCEs `reporting-date`\|`reportingDate`, `run-category`\|`runCategory`, `h3Region`\|`regionCode`), so the generated columns already reconciled what the rules could not. With A18 the database and the routing rules agree. **This widens matching versus legacy** (the legacy engine was case-insensitive but not hyphen-insensitive) and is therefore an expected shadow-parity diff — recorded as ASSUMPTION-9 in [docs/ems-seed0-assumptions.md](docs/ems-seed0-assumptions.md) |
...
- ~~Redundancy to resolve during the seed audit: post conditions exist in **both** the `filter.post` property and the map table — confirm the map is what `EventFilter.scala` actually evaluates, then retire the property.~~ **Resolved by A12 (§0): there is no redundancy.** The `filter.post` *property* feeds the Kafka **admission** gate; the *map table* is the **routing** authority ([old-ems/EventFilter.SCALA:68-70](old-ems/EventFilter.SCALA) vs [:35-50](old-ems/EventFilter.SCALA)). No `filter.post` row exists in [old-ems/properties.sql](old-ems/properties.sql), so the effective legacy admission set is the 7 PERSIST rows alone — which makes the `PERSIST ⊇ FORWARD` invariant below **load-bearing**: a FORWARD row matching an event no PERSIST row admits is dead. The sample rows carry obvious typos (`MERYVAL`, `context.date.*`) — treat them as illustrative, not literal.

**Mechanical translation** (legacy flat JSONPath-equality: array of objects = OR across rows; keys within an object = AND). **PERSIST rows only — see A14 (§0): the FORWARD rows cannot be translated mechanically** (the map-table condition grammar has no parser in the workspace), so their translation is a signed-off interpretation ([`docs/ems-seed0-assumptions.md`](docs/ems-seed0-assumptions.md)). **And per A13→A15, case is handled by the activation view, not by the rule text** — so the CEL below is illustrative in the wrong *case*: the shipped rules are written entirely in lower-case (`event.additionaldata.tenant == "frca"`), matching the legacy literals verbatim:
...
| `ems_registry_version{component,version}` | info-gauge (constant `1`) | **warn** divergence > 30 m (Phase B+) |
...
| 3 | **Substantially answered by `properties.sql`**, and the map-vs-property question **answered by A12 (§0)**: PERSIST = 7 `filter.persist` rows; FORWARD = CAPITAL → `orchestration_control_dag_capital` (8 rows) + NSFR → `orchestration_control_dag_liquidity` (disabled). The map is the *routing* authority and the property is a second *admission* input — neither is redundant, and no `filter.post` row exists in the evidence. **Remaining (still open):** collect per-environment row deltas; correct the sample typos (`MERYVAL`, `context.date.*`) against real rows. Both are sign-off lines in [`docs/ems-seed0-assumptions.md`](docs/ems-seed0-assumptions.md) — the `seed-0` migration does not invent them | `subscription` seed (`seed-0`); drop/forward parity targets |
